...
using System.Linq;
public class BlobGridbak : MonoBehaviour
{
    ...
    List<Color> colors = new List<Color> {Color.gray, Color.green, Color.magenta, Color.red, Color.yellow, Color.blue};
    private Node[,,] _grid;

    void Start()
    {      
        _grid = new Node[size, size, size];
...
// Update is called once per frame
    ...
    private List<Node> GetAboveInOrder(Node node) {
        ...
        if (node.Position.Y == size - 1) {
            return above;
        }
        for( int i = node.Position.Y + 1; i < size ; i++) {
    ...
    private IEnumerator DropOutInSeconds(List<Node> nodes, float t)
    {
        ...
        foreach (var node in nodes) {
            node.Blob.transform.localScale = node.Blob.transform.localScale * 1.2f;
            
            var material = node.Blob.GetComponent<Renderer> ().material;
            material.color = Color.Lerp(material.color, Color.white, 0.5f);
        ...
        foreach (var blobGrabInteractable in blobGrabInteractables) {
            Debug.Log("keep mask on blob was: " + (int)blobGrabInteractable.interactionLayerMask);
            blobGrabInteractable.interactionLayerMask = OutLayer();
            Debug.Log("keep mask on blob set: " + (int)blobGrabInteractable.interactionLayerMask);
        }
    ...
    private bool PositionInGrid(Position p) => p.X >= 0 && p.Y >= 0 && p.Z >= 0 && p.X < size && p.Y < size && p.Z < size;
